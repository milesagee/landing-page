...
import type { ActiveListing } from "@/lib/asihene-reflection-data";
import type { FamilyCard, FamilyData } from "@/lib/asihene-next-gen-data";
...
// Distinct from the reflection portal's "mams-reflection" namespace so
// draft state from the kids' surface does not collide with Sharon's
// draft state if both are viewed on the same browser.
const STORAGE_PREFIX = "mams-next-gen";
...
export function AsiheneNextGen({
  data,
  shareToken,
  contactId,
}: {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(
        `/api/family/${contactId}/submit?t=${encodeURIComponent(shareToken)}`,
        {
          method: "POST",
        ...
        {card.factBlock && (
          <div className="border-l-2 border-gold/40 pl-4 py-1">
            ...
            <p className="text-sm text-deep-teal/70">
              {card.factBlock.neighborhood} &middot; {card.factBlock.listPrice}{" "}
              &middot; {card.factBlock.status}
            </p>
