/**
 * /api/family/[contactId]/submit
 *
 * Records a single card submission from the Asihene next-gen family-input
 * portal (Stephen + Tanisha). Token-gated via URL query. Writes a GHL note
 * + card-specific tag to Stephen's contact and mirrors the submission to
 * Miles via GHL Conversations email.
 *
 * Forked from /api/reflection/[contactId]/submit/route.ts on 2026-05-31.
 */
...
const CARD_TAG: Record<string, string> = {
  "next-gen-opener": "asihene-next-gen-opener-input",
  "cheverton-onground": "asihene-cheverton-onground-read",
  "active-list-walk": "asihene-active-list-walk-priority",
  "honest-feedback": "asihene-honest-feedback",
};
...
export async function POST(req: Request, { params }: { params: Params }) {
  ...
  const noteBody = [
    `[Asihene Next-Gen] ${bothNames} submitted "${card.label} - ${card.headline}"`,
...
<html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#003F3F;max-width:640px;margin:0 auto;padding:20px;line-height:1.55;">
  <div style="background:#003F3F;color:white;padding:20px;border-radius:8px 8px 0 0;">
    <div style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#D4AF37;font-weight:600;">Asihene Next-Gen</div>
    <div style="font-size:22px;font-weight:700;margin-top:8px;font-family:'Fraunces',serif;">${bothNames} submitted the ${card.label.toLowerCase()} card.</div>
  </div>
  ...
  try {
    await ghlPost(`/conversations/messages`, {
      ...
      subject: `[Next-Gen] ${bothNames}: ${card.label}`,
      html: milesEmailHtml,
    });
...
export async function GET() {
  return NextResponse.json(
    {
      ok: false,
      hint: "POST with ?t=<token> + body { cardId, answers } to record a next-gen family submission.",
    },
    { status: 405 },
  );
}
